...
class Settings::ProvidersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "TrueLayer configure link breaks out of turbo frame when unconfigured" do
    Provider::FamilyConfig.where(family: families(:dylan_family), provider_key: "truelayer").destroy_all
    get settings_providers_url
    assert_response :success
    assert_select "a[href='#{new_provider_family_config_path(provider: "truelayer")}'][data-turbo-frame='_top']"
  end
  ...
  test "TrueLayer add connection submits via POST and bypasses turbo when configured" do
    ...
    get settings_providers_url
    assert_response :success
    assert_select "form[action='#{new_oauth_callbacks_path(provider: "truelayer")}'][method='post']" do

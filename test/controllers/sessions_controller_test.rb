...
class SessionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "redirects to OIDC end_session_endpoint with client_id when provider has client_id" do
    ...
    session_record.update!(id_token_hint: "fake-id-token")
    session_record.update!(oidc_provider: oidc_identity.provider)
  ...
  test "rejects SSO login when stored issuer does not match current provider config" do
    ...
    assert_redirected_to new_session_path
    assert_equal I18n.t("sessions.openid_connect.issuer_mismatch"), flash[:alert]
  end
